...
| \(L\otimes_KL\simeq_LL\times D_\alpha\) | marked-root decomposition | `primitiveTensorDecomposition`; adapter `cubicGenericCollisionEquiv` | Exact; `fst_cubicGenericCollisionEquiv` identifies first projection with \(\mu_L\). The adapter itself also does not carry the degree-three hypothesis. |
