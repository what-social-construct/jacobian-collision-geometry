...
\hypersetup{
  pdftitle={Secant Projectors and Boundary Inertia for Planar Keller Maps},
...
\title{Secant Projectors and Boundary Inertia for Planar Keller Maps}
...
\begin{abstract}
Let
\(F=(P,Q)\colon\A^2_{\C}\to\A^2_{\C}\) satisfy
\(\det JF=c\in\C^\times\).  A divided-difference secant matrix gives an
explicit idempotent in the coordinate ring \(C_F\) of the scheme of pairs
with the same image.  It splits off the diagonal and yields
\[
  C_F\cong \C[x_1,x_2]\times C_F^\circ,
  \qquad
  C_F^\circ\cong
  \frac{\C[x_1,x_2,y_1,y_2]}
       {(P(x)-P(y),\,Q(x)-Q(y),\,\det M_F)}.
\]
We then pass to the normal closure \(N/K\) of the induced function-field
extension and its finite normalization \(Z\).  Pulling the affine plane
back along the conjugate embeddings produces canonical boundary ideals
\(\mathfrak j_g\), while finite subgroups of \(\Gal(N/K)\) give fixed-locus
ideals \(\mathfrak a_C\).  We identify absence of divisorial ramification
with the absence of a common height-one component of the corresponding
fixed--moving boundary loci.  Purity and finite-\'etale rigidity then reduce
collision vanishing to this boundary condition.  A divisor-class argument
recovers the known normal/Galois case and excludes generic degree two.
Universal boundary separation is equivalent to the two-dimensional
Jacobian conjecture and is not proved here.  Lean formalizes the collision
and boundary constructions and the conditional divisorial endgame relative
to explicit normalization and standard geometric interfaces.
\end{abstract}
...
The complex two-dimensional Jacobian conjecture asks whether every
polynomial map
\[
  F=(P,Q)\colon\A^2_{\C}\longrightarrow\A^2_{\C},
  \qquad \det JF\in\C^\times,
\]
is a polynomial automorphism.  We study this question through the scheme
of ordered pairs \((x,y)\) satisfying \(F(x)=F(y)\).  Its diagonal records
equal source points; the remaining factor records the failure of
injectivity.

In dimension two the Keller condition makes this separation explicit.  A
divided-difference secant matrix produces an idempotent which splits the
collision scheme into its diagonal and off-diagonal parts.  The first half
of the paper develops this projector and its universal factorization
property.

The second half moves to the induced finite separable function-field
extension \(L/K\), its normal closure \(N/K\), and the normalization
\(Z\) of the target in \(N\).  Each conjugate embedding of \(L\) determines
an affine sheet in \(Z\) and a canonical deleted-boundary ideal.  For a
ramified divisor \(E\), the inertia group moves at least one conjugate
sheet.  Because every affine sheet is \'etale over the target, such a moved
center cannot remain affine; it must lie on the deleted boundary.

This observation yields an exact codimension-one dictionary.  Divisorial
ramification is absent precisely when no nontrivial fixed locus has a
common height-one component with all of its moving-sheet boundaries.  Once
height-one ramification has been eliminated, purity and finite-\'etale
rigidity trivialize the normal closure and collapse the collision scheme to
the diagonal.  Universal boundary separation is therefore another form of
\(JC(2)\); it is not proved here.  A separate divisor-class argument
recovers the known normal/Galois case and rules out generic degree two.

Section~\ref{sec:general-construction} recalls the collision, off-diagonal,
and normalization constructions used below.  Section
\ref{sec:planar-vanishing} gives the planar secant splitting, constructs
the conjugate boundary ideals, proves the divisorial dictionary, and states
the resulting boundary forms of \(JC(2)\).  The final part records a
prospective secant--trace mechanism separately from the completed
dictionary and endgame.

\section{General construction}
\label{sec:general-construction}

We recall the dimension-independent constructions needed for the planar
argument.  The collision ideal encodes pairs with the same image, the
diagonal ideal defines the distinguished equal-point locus, and the
normalization diagram organizes conjugate sheets and their inertia.  The
planar specialization will make the diagonal splitting explicit and locate
every possible divisorial obstruction on the deleted normalization boundary.
...
\subsection{The generic collision algebra}
...
\section{Planar boundary criteria}
...
behind \leanpath{CollisionIdeals/Planar/Research.lean}.  The focused
\leanpath{CollisionIdeals/Planar.lean} umbrella exports the basic and
explicit-secant APIs, the generic-degree-two theorem, and the planar
statement and equivalence modules.  The frame, Galois-pair,
conjugate-evaluation, boundary, and rigidity developments remain available
through the precise module paths named above.
\end{leanfiles}
...
For a planar Keller map, the secant determinant gives an explicit
idempotent decomposition of the collision ring into its diagonal and
off-diagonal factors.  On the normal closure, the same obstruction is
recorded divisorially: a ramified divisor can survive only when every
inertia-moving conjugate center lies on a deleted boundary.

The pulled-back boundary ideals and the fixed-locus ideals turn this into a
finite-group condition on height-one primes.  Once that condition excludes
divisorial ramification, purity and finite-\'etale rigidity make the normal
closure trivial and force \(\Obs(F)=0\).  The boundary divisor-class
argument recovers the known normal/Galois case and consequently excludes
generic degree two.  What remains is the nonnormal locus of generic degree
at least three.

Universal fixed--moving boundary separation is equivalent to \(JC(2)\),
so the equivalence itself is a reduction rather than a proof of the
conjecture.  The prospective secant--trace construction isolates a
concrete landing statement that could establish separation from the planar
polynomial data; no such universal landing theorem is asserted here.
