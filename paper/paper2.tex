...
a divided-difference secant matrix produces an explicit idempotent
...
\(\Obs(F)\neq0\), and for the Keller map above, hence \(p_F\neq0\).  On
the other hand, a separable
quadratic extension is Galois, and classical Keller--Galois rigidity makes
a hypothetical degree-two Keller map a polynomial automorphism.  Then
...
quadratic exclusion explicitly in the planar collision ring.
...
geometric and explicit: divided differences produce a secant determinant
and an idempotent \(p_F\) generating \(\Obs(F)\).  The second is generic
...
\begin{corollary}[Nonzero planar projector in degree two]
...
\begin{leanfiles}
The focused theorem spine is formalized in Lean~4 and Mathlib
