...
\usepackage{microtype,xcolor,titlesec,xurl,hyperref}
...
Section~\ref{sec:finite-boundary} isolates a finite first-order boundary
target for the remaining planar problem.
...
or inertia construction enters the degree-two proof.  The separate
boundary target below does not assume a generic-degree bound.
...
\section{Finite boundary data for planar vanishing}
\label{sec:finite-boundary}

The degree-two argument closes by Galois rigidity.  For the remaining
nonnormal case, the objective is still \(\Obs(F)=0\).  We now isolate a
finite first-order test on the target boundary.  Its algebraic criterion
is proved below; its vanishing for every planar Keller map remains open.
Throughout this section, \(F=(P,Q)\) is a planar Keller map, with no
generic-degree restriction.

\subsection{The reduced target boundary}

Let \(N\) be the normal closure of \(L/K\), fix the embedding
\(\iota\colon L\hookrightarrow N\), and put
\[
  G=\Gal(N/K),\qquad E=B[g(\iota(A)):g\in G]\subseteq N.
\]
Thus \(E\) is generated by all conjugate source algebras.  It is the
coordinate ring of the connected component of their fiber product
selected by these embeddings: the tensor product is regular and
\'etale over \(B\), and its selected generic factor is \(N\).
In particular, \(E/B\) is \'etale and \(\Frac(E)=N\).

The action of \(G\) on \(\Spec E\) is free.  Indeed, the scheme-theoretic
fixed locus of each \(g\ne1\) is open by unramifiedness and closed by
separatedness.  Since \(g\) acts nontrivially on \(\Frac(E)=N\), this
locus misses the generic point and is empty by connectedness.
The quotient \(\Spec E\to\Spec E^G\) is therefore a finite \'etale
\(G\)-torsor \cite{StacksFiniteFreeQuotient}.
Consequently \(\Spec E^G\to\Spec B\) is \'etale, separated, and
birational, hence an open immersion \cite{StacksBirationalEtaleOpen}.
Write its image as \(U_F\).  The complement of this dense affine open
has pure codimension one \cite{StacksAffineComplement}; since
\(B=\C[P,Q]\) is factorial, we may write
\[
  U_F=D(s),\qquad
  \mathfrak b_F:=I(\Spec B\setminus U_F)=(s),
  \qquad 0\ne s\in B\ \text{squarefree}.
\]
For an empty complement take \(s=1\).  The ideal \(\mathfrak b_F\)
depends only on \(U_F\), not on its defining equation, and
\(E^G=B[1/s]\).  It is a target-base boundary ideal, distinct from
the collision obstruction \(\Obs(F)\subseteq C_F\).

If \(\mathfrak b_F=B\), then \(E\) is finite over \(B\).  Since \(B\)
is Noetherian, the \(B\)-submodule \(\iota(A)\subseteq E\) is also finite.
Thus \(F\) is
finite \'etale; the associated covering of the simply connected space
\(\C^2\) has one sheet, so \(F\) is a polynomial automorphism and
\(\Obs(F)=0\).  The unresolved issue is excluding a nonempty boundary.

\subsection{A finite first-order criterion}

Let \(D_P,D_Q\) denote the extensions of \(\partial_P,\partial_Q\)
from \(B\) to \(K\) \cite{StacksDerivationLocalization}, and set
\[
  M_0=s^{-1}B,\qquad
  M_1=M_0+B\,D_P(1/s)+B\,D_Q(1/s)\subseteq K.
\]
The module \(M_0\) has one generator, \(M_1\) has at most three, and
\(M_1/M_0\) has at most two.  All finiteness statements here are over
\(B\), not finite-dimensionality over \(\C\).

\begin{proposition}[Finite boundary criterion]
\label{prop:finite-boundary-criterion}
For every \(0\ne s\in B\), the following conditions are equivalent:
\[
  \begin{gathered}
  M_1=M_0;\\
  D_P(M_0)\subseteq M_0,\qquad D_Q(M_0)\subseteq M_0;\\
  \partial_Ps,\partial_Qs\in(s);\\
  s\in B^\times.
  \end{gathered}
\]
For the reduced boundary equation above, these conditions say
\(U_F=\Spec B\).
\end{proposition}

\begin{proof}
Since each target derivative preserves \(B\), Leibniz reduces preservation
of \(M_0\) to its generator.  For \(j=P,Q\),
\[
  D_j(1/s)=-\frac{\partial_js}{s^2}\in s^{-1}B
  \quad\Longleftrightarrow\quad \partial_js\in(s).
\]
If \(s\) is nonconstant, every nonzero partial derivative has smaller
total degree than \(s\) and cannot be a multiple of it.  Both partials
would therefore have to vanish, forcing \(s\) to be constant in
characteristic zero, a contradiction.  Nonzero constants satisfy all
the displayed conditions.
\end{proof}

The same test has an intrinsic boundary formulation.  The conormal map
\cite{StacksConormalSequence}
\[
  \mathfrak b_F/\mathfrak b_F^2
  \longrightarrow
  \Omega_{B/\C}\otimes_B B/\mathfrak b_F
  \simeq (B/\mathfrak b_F)^2
\]
sends the generator \(s\bmod s^2\) to
\[
  (\partial_Ps\bmod s,\ \partial_Qs\bmod s).
\]
Its vanishing is equivalent to the criterion above.  This formulation
uses the existing conormal module of the boundary, not a replacement
for \(\Obs(F)\).

\subsection{The remaining comparison}

Galois symmetry supplies the collision relation.  For
\(g,h\in G\), write \(a=g(\iota(x))\) and \(b=h(\iota(x))\), where
\(x=(x_1,x_2)\).  Since both automorphisms fix \(K\),
\[
  F(a)=F(b),\qquad M_F(a,b)(a-b)=0.
\]
If \(a\ne b\), then \(\det M_F(a,b)=0\).  These identities hold in
\(N\) without normality of \(L/K\).  The coordinate-descent condition
\(g(\iota(x_i))\in\iota(L)\) for every \(g\in G\) and \(i=1,2\)
would place them in the marked original field.
Merely rewriting the secant relation in \(L\) supplies no
boundary comparison.
Once that descent condition is proved, normality and
Theorem~\ref{thm:keller-galois-rigidity} already give invertibility.

The missing Keller-specific statement is
\[
  \partial_P\mathfrak b_F\subseteq\mathfrak b_F,\qquad
  \partial_Q\mathfrak b_F\subseteq\mathfrak b_F.
\]
It asks only for the two first-order classes just displayed to vanish.
The polynomial inverse-Jacobian frame differentiates target polynomials
by \(\partial_P,\partial_Q\), and its extension preserves every conjugate
source algebra.  These identities do not yet prove the displayed
containments: preservation of \(B[1/s]\) allows
\(D_j(1/s)=-\partial_js/s^2\), with a possible double pole.

Likewise, clearing finitely many secant coefficients and their first
derivatives does not prove \(M_1=M_0\).  Their denominator ideal lies
in the integral closure of \(B\) in \(N\), whereas \(\mathfrak b_F\)
lies in \(B\); no comparison identifying those ideals is asserted.
The next step is a finite comparison from the actual global
Keller--secant data to these conormal classes.  It is not a bound on
an entire pole tower, and no such comparison is assumed in the
degree-two theorem.
...
For Section~\ref{sec:finite-boundary}, the algebra \(E\) is constructed
from the conjugate source coordinates in
\leanpath{CollisionIdeals/General/Galois/ConjugateAlgebra.lean}.
Conjugate-polynomial membership, Galois stability, and finite type are
proved; finite type does not mean finite as a \(B\)-module.
The simple-pole preservation criterion is proved in
\leanpath{CollisionIdeals/General/Normalization/SimplePole.lean}.
The conormal components are expressed using Mathlib's existing
\texttt{Ideal.Cotangent} in
\leanpath{CollisionIdeals/General/Normalization/ConormalDerivation.lean}.
The invariant-open identification and its reduced target boundary have
not yet been instantiated in Lean.  The Keller-specific vanishing of the two
conormal classes is open; the algebraic criteria do not prove it.
\end{leanfiles}
...
input.  For the broader planar problem,
Section~\ref{sec:finite-boundary} isolates a finite first-order boundary
criterion; deriving its vanishing from the global Keller construction
remains the research target.
