...
of the scheme of pairs with the same image and define its diagonal by the
ideal \(\Obs(F)\subseteq C_F\).  Put
\(B=\C[F_1,\dots,F_n]\), \(K=\Frac(B)\), and
\(L=\C(x_1,\dots,x_n)\).
We show that when \(n=3\) and \([L:K]=3\), every Keller map is a
...
\[
  ...
  \Gal(N/K)\simeq S_3.
\]
Here \(N\) is the normal closure of \(L/K\).  This describes every possible
generic-degree-three counterexample; it neither asserts existence nor
classifies counterexamples of other generic degrees.
\end{abstract}
