...
these constructions to cubic extensions.  The closing low-degree corollary
records the input used by the planar sequel.
...
finiteness.  The predicate \leanpath{KellerEtaleBridge} exposes the
Jacobian-criterion part of Proposition
\ref{prop:keller-generic-geometry} as a theorem target;
\leanpath{KellerEtaleBridge.toKellerFlatBridge} now derives
\leanpath{KellerFlatBridge} from it.  Lean does not yet derive the étale
bridge, generic finiteness, or separability from \leanpath{IsKeller}.
...
\[
  K\otimes_BC_F\simeq_K L\times N.
\]
Counterexamples of other generic degrees lie outside the present analysis.
...
The formal verification establishes a constant nonzero Jacobian determinant
and distinct rational points with
...
The formalization uses Lean~4.28.0 and Mathlib~v4.28.0
\cite{Lean4,Mathlib}.  Its Palomar-facing declaration is
\leanpath{CollisionIdeals.Palomar.PaperOne.genericDegreeThreeS3Collision},
checked by
\texttt{lake build CollisionIdeals.Palomar.PaperOne.Solution}.
...
Paper~II specializes this low-degree boundary to ambient dimension two and
makes the quadratic collision contradiction explicit through the planar
secant projector.

\paragraph{Acknowledgment.}
OpenAI Codex (GPT-5.6 Sol) assisted with the Lean formalization and
manuscript.  The author reviewed and takes responsibility for all statements, proofs,
